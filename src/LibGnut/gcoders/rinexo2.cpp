    ...
    if( ! _fname.empty() ){
      ...
      if( guess != marker ){
        ...
        if( guess.substr(0,4) !=  marker.substr(0,4) ){
		    if (_log) _log->comment(2, "rinexo2", "Warning: 4-CHAR: MARKER_NAME ["+marker+"] not equal to site from filename ["+guess+"]");
        }
      }
    }
  ...
  // -------- "# / TYPES OF OBSERV" --------
    ...
    if( ! _csys.empty() && _csys[0] != ' ' ){
      ...
      for(int ii=0; ii<num; ++ii){
        ...
        // new record
		_mapobs[_csys].push_back(make_pair(obs, fact)); // changed since some situations have same obstype skipping may cause bugs
...
int t_rinexo2::_check_head()
{
  ...
  if( _rnxhdr.rnxsys() == ' '    ){ if (_log) _log->comment(2, "rinexo2", "Error: RINEX SYS not available!");   _irc++; } // mandatory
  if( _rnxhdr.rnxver().empty()   ){ if (_log) _log->comment(2, "rinexo2", "Error: RINEX VERS not available!");  _irc++; } // mandatory
  if( _rnxhdr.program().empty()  &&
      _rnxhdr.program().empty()  ){ if( _log ) _log->comment(2,"rinexo2","Error: PGM/RUNBY not available!");       _irc++; } // mandatory
  if( _rnxhdr.observer().empty() &&
      _rnxhdr.agency().empty()   ){ if( _log ) _log->comment(2,"rinexo2","Error: OBSERVER/AGENCY not available!"); _irc++; } // mandatory
  if( _rnxhdr.markname().empty() ){ if( _log ) _log->comment(2,"rinexo2","Error: MARKER NAME not available!"); _irc++; } // mandatory
  if( _rnxhdr.recnumb().empty()  ){ if( _log ) _log->comment(2,"rinexo2","Error: REC NUMB not available!");    _irc++; } // mandatory
  if( _rnxhdr.rectype().empty()  ){ if( _log ) _log->comment(2,"rinexo2","Error: REC TYPE not available!");    _irc++; } // mandatory
  if( _rnxhdr.recvers().empty()  ){ if( _log ) _log->comment(2,"rinexo2","Error: REC VERS not available!");    _irc++; } // mandatory
  if( _rnxhdr.antnumb().empty()  ){ if( _log ) _log->comment(2,"rinexo2","Error: ANT NUMB not available!");    _irc++; } // mandatory
  if( _rnxhdr.anttype().empty()  ){ if( _log ) _log->comment(2,"rinexo2","Error: ANT TYPE not available!");    _irc++; } // mandatory
  if( _rnxhdr.mapobs().size()==0 ){ if( _log ) _log->comment(2,"rinexo2","Error: OBS TYPE not available!");    _irc++; } // mandatory
  if( _rnxhdr.aprxyz() == zero   ){ if( _log ) _log->comment(2,"rinexo2","Error: APR XYZ not available!");     _irc++; } // mandatory
  if( _rnxhdr.first() == FIRST_TIME
                                 ){ if( _log ) _log->comment(2,"rinexo2","Error: FIRST TIME not available!");  _irc++; } // mandatory
  
 if( _rnxhdr.interval() <= 0    ){ if( _log ) _log->comment(2,"rinexo2","Warning: INTERVAL not available!");        } // optional --> warning
 if( _rnxhdr.marknumb().empty() ){ if( _log ) _log->comment(2,"rinexo2","Warning: MARKER NUMB empty field!");       } // optional --> warning
